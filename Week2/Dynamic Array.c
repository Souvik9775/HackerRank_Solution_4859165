...
int* dynamicArray(int n, int queries_rows, int queries_columns, int** queries, int* result_count) {
   int** seqList = malloc(n * sizeof(int*));       
    int* seqSize = calloc(n, sizeof(int));          
    for (int i = 0; i < n; i++) {
        seqList[i] = NULL;                    
        seqSize[i] = 0;
    }
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            ...
            seqList[seqIndex] = realloc(seqList[seqIndex], (seqSize[seqIndex] + 1) * sizeof(int));
            seqList[seqIndex][seqSize[seqIndex]] = y;
            seqSize[seqIndex]++;
        }
        else if (type == 2) {
            int elementIndex = y % seqSize[seqIndex];
