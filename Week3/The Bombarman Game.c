 ...
 *
 */
 char** makeFull(int r, int c) {
    char** res = (char**)malloc(r * sizeof(char*));
    for (int i = 0; i < r; i++) {
        res[i] = (char*)malloc((c + 1) * sizeof(char));
        for (int j = 0; j < c; j++) res[i][j] = 'O';
        res[i][c] = '\0';
    }
    return res;
}
char** copyGrid(char** grid, int r, int c) {
    char** res = (char**)malloc(r * sizeof(char*));
    for (int i = 0; i < r; i++) {
        res[i] = (char*)malloc((c + 1) * sizeof(char));
        strcpy(res[i], grid[i]);
...
int r = grid_count;
    ...
    *result_count = r;
    ...
    char** result = makeFull(r, c);
    explode(grid, result, r, c);
    ...
    char** temp = copyGrid(result, r, c);
    char** result2 = makeFull(r, c);
    explode(temp, result2, r, c);

    
    for (int i = 0; i < r; i++) free(temp[i]);
    free(temp);
